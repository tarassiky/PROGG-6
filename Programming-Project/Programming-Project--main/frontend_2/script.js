// ========== КОНФИГУРАЦИЯ ==========
const USE_MOCK = false;   // true = заглушки, false = реальный бэкенд
...
// ========== АВТОПОДСЧЁТ ДОХОД / СУММА КРЕДИТА ==========
// Когда пользователь меняет доход или сумму кредита — автоматически
// пересчитываем отношение loan_amnt / person_income и вставляем в поле
function updateLoanPercentIncome() {
  ...
  if (income > 0 && loanAmnt > 0) {
    const ratio = (loanAmnt / income).toFixed(2);
    field.value = ratio;
  } else {
    field.value = '0.00';
  }
...
document.getElementById('loanForm').addEventListener('submit', async (e) => {
  ...
  // Проверяем возраст до отправки запроса
  const age = parseFloat(document.getElementById('person_age').value);
  if (age < 18) {
    alert('⚠️ Предупреждение: возраст заявителя должен быть не менее 18 лет. Выдача ипотеки несовершеннолетним невозможна.');
    return;
  }
...
// Показываем имя выбранного CSV-файла рядом с кнопкой
...
// Подписываемся на изменения дохода и суммы кредита для автоподсчёта
...
// Считаем соотношение при загрузке страницы (для дефолтных значений)
