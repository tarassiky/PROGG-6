...
from iteration1_integrate import integrate   # переиспользуем базовую функцию
...
def benchmark():
    ...
    for jobs in (2, 4, 6, 8):
        t = timeit.timeit(
            f'integrate_threads(math.sin, 0, math.pi/2, n_jobs={jobs}, n_iter={n_iter})',
            globals=globals(), number=1
        )
        print(f"Потоки, jobs={jobs}: {t:.6f} сек")
